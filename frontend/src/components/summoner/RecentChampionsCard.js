            ...
            return (
              <div key={c.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-hover transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-500"><span className="text-blue-400">{c.wins}W</span> <span className="text-red-400">{c.losses}L</span> · {ak} KDA</p>
                </div>
                <span className={`text-xs font-bold font-mono ${wr >= 50 ? "text-blue-400" : "text-red-400"}`}>{wr}%</span>
              </div>
            );
          })}
        </div>
