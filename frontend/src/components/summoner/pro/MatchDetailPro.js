  ...
  return (
    <div className={`flex items-center gap-1.5 px-2 py-1.5 ${isMe ? "bg-cyan-500/5" : ""} hover:bg-hover transition-colors`}>
      ...
      <div className="relative flex-shrink-0">
        <img src={p.champion.image} alt="" width={30} height={30} className="rounded-md" />
        <span className="absolute -bottom-1 -right-1 text-[7px] bg-card text-gray-400 px-0.5 rounded border border-edge font-mono">{p.champLevel}</span>
      </div>

      {/* Spell + Rune */}
      <div className="hidden sm:flex flex-col gap-0.5 flex-shrink-0">
        {p.spells?.[0]?.image && <img src={p.spells[0].image} alt="" width={14} height={14} className="rounded-sm" title={p.spells[0].name} />}
        {p.spells?.[1]?.image && <img src={p.spells[1].image} alt="" width={14} height={14} className="rounded-sm" title={p.spells[1].name} />}
      </div>
      <div className="hidden sm:block flex-shrink-0">
        <RuneTooltip runes={p.runes} keystoneSize={16} subTreeSize={12} />
      </div>

      {/* İsim + rank */}
      <div className="w-[96px] min-w-0 flex-shrink-0">
        <p className={`text-[11px] truncate leading-tight ${p.isBot ? "text-gray-500 italic" : isMe ? "text-cyan-300 font-semibold" : "text-gray-200"}`}>
          {p.summonerName}
        </p>
        <div className="flex items-center gap-1 leading-tight">
          {p.tier && <img src={rankBadgeUrl(p.tier)} alt="" width={12} height={12} />}
          <span className="text-[9px] text-gray-500 truncate">{p.isBot ? "BOT" : rank || "—"}</span>
        </div>
      </div>
      ...
      <div className="w-[58px] text-center flex-shrink-0">
        ...
        <p className={`text-[9px] font-semibold leading-tight ${kdaColor(p.kda)}`}>
          {p.kda === "Perfect" ? "Perfect" : `${p.kda.toFixed(1)}`}
          {mk && <span className="ml-0.5 text-[7px] text-amber-400">{mk[0]}{mk.length > 1 ? mk.slice(1, 2).toLowerCase() : ""}</span>}
        </p>
      </div>

      {/* CS / KP */}
      <div className="w-[52px] text-center text-[9px] text-gray-500 flex-shrink-0 leading-tight">
      ...
      <div className="flex items-center gap-0.5 flex-shrink-0">
        {(p.items || []).slice(0, 7).map((item, i) => <ItemTooltip key={i} item={item} size={20} />)}
      </div>

      {/* Hasar barı */}
      <div className="flex-1 min-w-[44px] hidden lg:block">
        <div className="flex items-center gap-1">
          <span className="text-[9px] text-red-400/80 font-mono w-9 text-right">{fmtDmg(p.damage)}</span>
      ...
      {/* ELW */}
      <span className="w-7 text-right text-[11px] font-bold flex-shrink-0" style={{ color: scoreColor(sc) }}>
...
function TeamBlock({ team, searchedPuuid, maxDmg }) {
  ...
  return (
    <div className={`rounded-lg overflow-hidden border ${isWin ? "border-blue-500/25" : "border-red-500/25"}`}>
      <div className={`px-3 py-1.5 flex items-center justify-between ${isWin ? "bg-blue-500/10" : "bg-red-500/10"}`}>
        <div className="flex items-center gap-2">
          <span className={`text-xs font-bold ${isWin ? "text-blue-400" : "text-red-400"}`}>{isWin ? "Zafer" : "Yenilgi"}</span>
          <span className="text-[10px] text-gray-500">{team?.info?.totalKills}/{deaths}/{assists}</span>
        </div>
        <span className="text-[10px] text-gray-500">{fmtGold(team?.info?.totalGold || 0)} gold</span>
      </div>
      <div className="divide-y divide-edge/15">
        {team?.players?.map((p, i) => <PlayerRow key={p.puuid + i} p={p} isMe={p.puuid === searchedPuuid} maxDmg={maxDmg} />)}
      </div>
    </div>
  );
}
...
export default function MatchDetailPro({ matchId, puuid }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <TeamBlock team={t1} searchedPuuid={puuid} maxDmg={maxDmg} />
      <TeamBlock team={t2} searchedPuuid={puuid} maxDmg={maxDmg} />
    </div>
  );
}
