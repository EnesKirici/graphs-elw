...
class BackfillMatchupRole extends Command
{
    ...
    public function handle(BuildAggregationService $agg): int
    {
        ...
        while (true) {
            ...
            // [patch|champ|pos|opp|oppPos] => [patch, champ, pos, opp, oppPos, n, taken, hs, cc]
            $acc = [];
            foreach ($batch as $matchId) {
                ...
                foreach ($agg->matchupRows($data) as [$patch, $champ, $pos, $opp, $oppPos, , , , , , , $taken, $hs, $cc]) {
                    ...
                    $acc[$key] ??= [$patch, $champ, $pos, $opp, $oppPos, 0, 0, 0, 0];
                    $acc[$key][5]++;         // n_role
                    $acc[$key][6] += $taken; // sum_taken
                    $acc[$key][7] += $hs;    // sum_heal_shield
                    $acc[$key][8] += $cc;    // sum_cc
                }
                $done++;
            }
            ...
            foreach (array_chunk(array_values($acc), 500) as $rows) {
                ...
                foreach ($rows as $r) {
                    $ph[] = '(?,?,?,?,?,?,?,?,?,?,?)';
                    array_push($bind, $r[0], $r[1], $r[2], $r[3], $r[4], $r[5], $r[6], $r[7], $r[8], $now, $now);
                }
                ...
                DB::statement(
                    ...
                    . ' n_role, sum_taken, sum_heal_shield, sum_cc, created_at, updated_at) VALUES '
                    . implode(',', $ph)
                    ...
                    . ' sum_cc = sum_cc + VALUES(sum_cc), updated_at = VALUES(updated_at)',
                    $bind,
                );
            }
