...
## Sıradaki (öncelik)
1. LabelEngine'i bağla: enrichment aggregate'leri + enemyChamps + evaluate çağrısı + frontend renkli gösterim.
2. Admin paneli `/admin/settings/labels` (bağlam sekmeli, aç/kapa+renk+eşik).
3. Şampiyon istatistikleri kart ön yüzüne.
4. Duo tespiti (ortak maç sezgiseli).
5. Frontend priority-sıralı fetch + 6+ deferral (worker/retry).
6. **Production key sonrası:** 10 oyuncu tam çek → güvenilir rol + tam etiket + tam kapsama + duo.
